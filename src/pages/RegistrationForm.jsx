...
const COMMITTEE_LIMITS = {
  ...
  'Economic and Social Council (ECOSOC)': 30,
              ...
              {adminTab === 'database' && (
                <div className="flex flex-col gap-4">
                  ...
                  {(() => {
                    ...
                    const COMM_CAPS = COMMITTEE_LIMITS;
                    const TOTAL_CAPACITY = Object.values(COMMITTEE_LIMITS).reduce((a, b) => a + b, 0);
                      ...
                      <div className="text-right text-xs">
                        ...
                        <p className="text-sm font-bold text-white mt-0.5">
                          {(() => {
                            ...
                            const totalLimit = COMMITTEE_LIMITS[committee] || pool.length;
                  ...
                  <div className="border border-[#DCA843]/15 rounded-lg p-5 bg-black/25">
                    ...
                    >
                      ...
                      {getAvailableCommittees().map((avail) => (
                        ...
                        >
                          {avail.name} ({Math.max(0, avail.limit - avail.filled)} left) {avail.isClosed ? '(CLOSED)' : (avail.isFull ? '(FULL)' : '')}
                        </option>
                      ))}
                    </select>
                                ...
                                <div>
                                  ...
                                  >
                                    ...
                                    {getAvailableCommittees(idx).map((avail) => (
                                      ...
                                      >
                                        {avail.name} ({Math.max(0, avail.limit - avail.filled)} left) {avail.isClosed ? '(CLOSED)' : (avail.isFull && del.selectedCommittee !== avail.name ? '(FULL)' : '')}
                                      </option>
                                    ))}
                                  </select>
